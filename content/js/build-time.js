...
function setBuildDate() {
  ...
  if (buildDateElement && buildDate && !buildDateElement.getAttribute('set')) {
    const dateObject = new Date(buildDate * 1000);
    var dateArray = [];
    ...
    const dateString = dateArray['YYYY'] + '-' + dateArray['MM'] + '-' + dateArray['DD'] + ' ' + dateArray['HH'] + ':' + dateArray['mm'] + ':' + dateArray['ss'] + (timeZone? ' ' + timeZone : '');
