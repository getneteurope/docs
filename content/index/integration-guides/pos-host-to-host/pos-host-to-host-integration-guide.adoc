...
|===
h| Payment Mode 
a|Credit card: link:https://docs.getneteurope.com/PaymentMethods.html#PaymentMethods_PaymentMode_OnlineBankTransfer[Online Bank Transfer], link:https://docs.getneteurope.com/PaymentMethods.html#PaymentMethods_PaymentMode_OfflineBankTransfer[Offline Bank Transfer]
...
For link:https://docs.getneteurope.com/Glossary.html#Glossary_TransactionType[transaction type] details which are not given here, look at link:https://docs.getneteurope.com/AppendixB.html[Transaction Types].

[cols=",,"]
|===
| Transaction Type | Description | Link to the samples
m| authorization | Reserves funds from the cardholder's account. A ``capture`` on this transaction must be conducted within 3-30 days, depending on the acquirer and card brand. | link:#POSHostToHostIntegrationGuide_Samples_authorization[``authorization`` samples]
m| capture-authorization | Takes funds from the cardholder's account. Must follow an ``authorization``. | link:#POSHostToHostIntegrationGuide_Samples_captureauthorization[``capture-authorization`` samples]
m| credit | Moves funds from the merchant to the cardholder's account. | link:#POSHostToHostIntegrationGuide_Samples_credit[``credit`` samples]
m| purchase | Takes funds from the cardholder's account. This is a one-step process to conduct 2 transactions: ``authorization`` and ``capture``. | link:#POSHostToHostIntegrationGuide_Samples_purchaseWithPIN[``purchase`` samples (with PIN)] +
link:#POSHostToHostIntegrationGuide_Samples_purchaseWithoutPIN[``purchase`` samples (without PIN)] +
link:#POSHostToHostIntegrationGuide_Samples_purchaseWithTrack2AndICC[``purchase`` samples (with track2 data and ICC data)]
m| refund-capture | Returns funds to the cardholder's account, referring to an eligible ``capture``. | link:#POSHostToHostIntegrationGuide_Samples_refundcapture[``refund-capture`` samples]
m| refund-purchase | Returns funds to the cardholder's account, referring to an eligible ``purchase``. | link:#POSHostToHostIntegrationGuide_Samples_refundpurchase[``refund-purchase`` samples]
m| settlement | Requests the acquirer to close all pending transactions. Used, for example, at the end of the day when the shop closes. | link:#POSHostToHostIntegrationGuide_Samples_settlement[``settlement`` samples]
m| void-authorization | Frees reserved funds from the cardholder's account due to an ``authorization``. | link:#POSHostToHostIntegrationGuide_Samples_voidauthorization[``void-authorization`` samples]
m| void-credit | Frees reserved funds from the merchant's account due to a ``credit``. | link:#POSHostToHostIntegrationGuide_Samples_voidcredit[``void-credit`` samples]
m| void-purchase | 	Frees reserved funds from the cardholder's account due to a ``purchase``. | link:#POSHostToHostIntegrationGuide_Samples_voidpurchase[``void-purchase`` samples]
|===
...
[cols="1,1"]
|===
| *URLs (Endpoints) for Test Environment* | ``https://api-test.getneteurope.com/engine/rest/payments/``
| *URLs (Endpoints) for Production Environment* | ``https://api.getneteurope.com/engine/rest/payments/``
| *User Name* | 515225-GetnetEuropeTEST
| *Password* | 4cHLRE-Q7YcAP
| *Test Cards* | For details on credit cards you can use for testing, see link:https://docs.getneteurope.com/CC_Main.html#API_CC_TestCards[Test Cards].
|===

[NOTE]
====
* Contact link:https://docs.getneteurope.com/ContactUs.html[Merchant Support] for production environment credentials.
* For additional test credentials, see link:https://docs.getneteurope.com/CC_Main.html#CreditCard_TestCredentials[Test Credentials].
====
...
[cols="4m,2,2,1,6a"]
|===
| Field | link:https://docs.getneteurope.com/RestApi_Fields.html#APIRef_FieldDefs_Cardinality[M/O] | link:https://docs.getneteurope.com/RestApi_Fields.html#APIRef_FieldDefs_DataTypes[Data Type] | Size | Description
...
For a list of supported card types, see link:https://docs.getneteurope.com/CC_Main.html#AppendixD[Card Types].
...
Mandatory if ``card.account-number`` is not specified. Unique per instance.
...
For a complete list of status codes, see link:https://docs.getneteurope.com/StatusCodes.html#StatusCodes_InDetail[Status Codes in Detail].
