...
. *Render the seamless payment form:* {payment-page-v2-abbr} returns an initial response URL.
+
====
...
to the authentication page, and then to a {payment-page-v2-abbr} success- or
...
{payment-page-v2-abbr} returns base64 encoded payment data. It is _highly recommended_ that
...
For 3D Secure transactions, {payment-page-v2-abbr} in Seamless Mode automatically
...
can be found in <<PPSolutions_PPv2_ConfigureRedirects, Configuring Redirects and IPNs for {payment-page-v2-abbr}>>.
====
...
status. +
*Please consult <<PPSolutions_PPv2_PPv2Security, {payment-page-v2-abbr} Security>> for details and examples of response signature verification.*

//-

In Seamless Mode, the {payment-page-v2-abbr} sends the final response containing the payment data to either of the following destinations, depending on the
payment mode.

[#PPv2_Seamless_Integration_Example_Parse_3DS]
*3D Secure Credit Card Payment*

The {payment-page-v2-abbr} sends the final response containing the payment data to the
...
payment, please consult *<<PPSolutions_PPv2_PPv2Security, {payment-page-v2-abbr} Security>> for details.*
...
2+|transaction-id                             
|String  
|A unique identifier assigned to every transaction(by {payment-provider-name}). Used when searching for or referencing to it later.
...
The {payment-page-v2-abbr} sends the final response containing the payment data *as part of the ``onSuccess/onError`` function.* Use the response to redirect the
