...
<<PPv2_SupportedPaymentMethods, Payment Methods with {payment-page-v2-abbr}>>.
...
|merchant-account-id m|value |String |Required |36 |A unique identifier assigned to every merchant account (by {payment-provider-name}).
...
Alternatively, check out <<PPv2_SupportedPaymentMethods, Payment Methods with {payment-page-v2-abbr}>>
...
[source,html,subs=attributes+]
----
<script type="text/javascript">{payment-page-function-short}.embeddedPayUrl("https://{pp-test-instance-hostname}/?wPaymentToken=f0c0e5b3-23ad-4cb4-abca-ed80a0e770e7")</script>
----
...
|merchant-account-id m| value | String | A unique identifier assigned to every merchant account (by {payment-provider-name}).
...
For payment-method-specific requests, head over to the <<PPv2_SupportedPaymentMethods, Payment Methods with {payment-page-v2-abbr}>>.
====
