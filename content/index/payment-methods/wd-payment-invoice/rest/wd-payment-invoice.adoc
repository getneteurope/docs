...
_{payment-provider-name} Payment on Invoice_ is a type of <<PaymentMethods_PaymentMode_OfflineBankTransfer, Offline Bank Transfer>>.
...
This table illustrates how _{payment-provider-name} Payment on Invoice_ notifications are encoded and which formats and methods can be
...
|authorization    | Performs a creditworthiness check at the moment when
                    ...
                    the basket.
                                | <<API_PaymentInvoice_Samples_GuaranteedInvoice, _{payment-provider-name} Payment on Invoice_ samples>>
...
image::images/11-11-guaranteed-invoice/guaranteed_invoice_workflow.png[{payment-provider-name} Payment on Invoice Workflow, width=950, align="center"]

. Consumer adds items to shopping basket.
. Consumer selects the payment method _{payment-provider-name} Payment on Invoice._
...
. Merchant sends a ``capture`` request to {payment-provider-name} when the items
are ready for delivery.
. After a successful ``capture`` request, {payment-provider-name} or Merchant generates
...
a test environment to get an approval to go live. {payment-provider-name} checks and
...
===== Simple Process Chain for {payment-provider-name} Payment on Invoice
...
===== Complex Process Chain for {payment-provider-name} Payment on Invoice
...
*C* for sending a request for the payment method _{payment-provider-name} Payment on Invoice_ (complete field
...
|Alphanumeric
|30
|This is the type for a transaction: ``authorization``, ``capture``, ``void-authorization``, ``refund-capture`` and ``credit`` are supported for _{payment-provider-name} Payment on Invoice_.
...
|Alphanumeric
|
|{payment-provider-name} Bank creditor identifier, provided by {payment-provider-name} Support
...
|Alphanumeric
|
|Mandate ID will be generated by {payment-provider-name}. Please use the following statement as placeholder: "Wird nach Kaufabschluss übermittelt"
...
==== Device Fingerprint ({payment-provider-name} Payment on Invoice)

[#API_PaymentInvoice_DeviceFingerprint_GuaranteedInvoice_Introduction]
===== Introduction

_Device Fingerprint ({payment-provider-name} Payment on Invoice)_ is a method of fraud
...
===== Data Transfer to {payment-provider-name}

Please transfer the ``deviceIdentToken`` within the authorization request to
{payment-provider-name}. You can find a corresponding XML sample for either <<API_PaymentInvoice_Samples_GuaranteedInvoice, {payment-provider-name} Payment on Invoice>> or <<API_PaymentDirectDebit_Samples_DirectDebit, {payment-provider-name} Payment Guaranteed Direct Debit>>.

[#API_PaymentInvoice_RegularConsumers]
==== Regular Consumers

Regular consumers who pay their bills on time are important capital for
each merchant. With {payment-provider-name}, merchants can use this capital as a
payment method.

To accomplish that, merchants submit a list with the order and
payment history of their consumers to {payment-provider-name}.
...
During checkout, the consumer has to agree to {payment-provider-name}'s general terms
...
notice] and https://www.wirecardbank.com/privacy-documents/zusatzliche-geschaftsbedingungen-fur-wirecard-zahlarten/[additional terms and conditions] for {payment-provider-name} payment types and that I accept their validity.
a|
...
When consumers use _{payment-provider-name} Payment on Invoice,_
...
===== _authorization_ Request to {payment-provider-name}
