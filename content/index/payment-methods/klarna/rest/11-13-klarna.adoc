...
NOTE: From September 2020 on, this version of Klarna will no longer be supported. Use Klarna Payments instead.
...
end consumer. The order amount will be paid out to you
...
on the {payment-provider-name} test environment before going live. <<ContactUs, Merchant Support>> provides the description of the tests to be executed to you by email. Once the test results are verified and confirmed that they are passed successfully you will receive access credentials for the production environment and can proceed with the
integration.
...
==== authorization
image::images/diagrams/klarna_authorization.svg[Klarna Workflow authorization,height=250]

- You start with an _authorization_ of the payment by sending a
...
- {payment-provider-name} performs a risk check for the payment using Klarna service
call _reserve_amount_.
- If Klarna authorizes the payment, it is guaranteed for you
once captured.
- If Klarna cannot guarantee the payment, the _authorization_ is
rejected with a corresponding error code.
- The result of the risk check is returned to you - with the status code inside
the _authorization_ response.
...
image::images/diagrams/klarna_pending-authorization.svg[Klarna Workflow pending-authorization,height=250]
...
- {payment-provider-name} takes care of the requesting the actual status of a pending
_authorization_ and will notify you system about the status
...
_success/failed_ and status code and description is sent to your
system asynchronously.
- This new _authorization_ transaction is stored in the system and has
the parent transaction ID referencing the _pending-authorization_
transaction.
...
a technical error. You have the possibility to check the status
...
- Please contact {payment-provider-name} Customer Support if a _pending-authorization_
...
image::images/diagrams/klarna_authorization-renewal.svg[Klarna Workflow authorization-renewal,height=250]
...
image::images/diagrams/klarna_adjust-authorization.svg[Klarna Workflow adjust-authorization,height=250]
...
- You send an _adjust-authorization_ transaction with parent
transaction ID referencing the previous _authorization_ or
_adjust-authorization_ transaction. {payment-provider-name} is using Klarna call
_update._
...
transaction.
- The result of the adjustment is returned to you - with the status code inside
the _adjust-authorization_ response.
...
image::images/diagrams/klarna_void-authorization.svg[Klarna Workflow void-authorization,height=250]
...
- You send a _void-authorization_ transaction with parent
transaction ID referencing the _authorization_/_adjust-authorization_
transaction to be cancelled. {payment-provider-name} is using Klarna call
_cancel_reservation_.
- A _pending-authorization_ can be cancelled as well.
...
image::images/diagrams/klarna_capture-authorization.svg[Klarna Workflow capture-authorization,height=250]

NOTE: In case of the payment instructions, you are responsible to send
the invoice to your consumers. The Klarna bank account
requisites, where the payment should be provided to, can be taken from
the payment instructions.

- You must capture an _authorization_ when the goods or services
are ready to be delivered.
- This call will inform {payment-provider-name} and Klarna using _activate_ call that
the order is being shipped.
- Depending on your configuration Klarna will then send an
...
- To capture an order you send a _capture-authorization_
...
image::images/diagrams/klarna_refund-capture.svg[Klarna Workflow refund-capture,height=250]

- If a consumer returns all or some items, you need to issue a
...
{payment-provider-name} is using the Klarna call _credit_part_.
...
additional fees that may occur at your side (shipping fee or
handling fee) you have to provide new order item(s) with the type
...
In case of fraud if you have violated the Klarna shipping
...
paid out to you will be reduced by the VAT value. A new
...
higher than the total amount of all captured articles. You can
...
cancellation of the remaining (not captured) articles, you have
the possibility to change (decrease) the amount of the voucher before
activation using _adjust-authorization_ request. After a capture it will
not be possible to decrease the amount of the already captured voucher.

Voucher is refunded like all other articles (using _refund-capture_ call
...
agreements and general approving for you by {payment-provider-name} and Klarna
Business Underwriting beforehand, otherwise the call would be rejected.
- In case of a partial refund, you might want to reduce a given
...
150 Euro, therefore you do not want to give a voucher anymore or
want to decrease the voucher.
...
|merchant-account-id |M |M |Alphanumeric |36 2+|Unique identifier for a
merchant account 

|request-id |M |M |Alphanumeric |150 |This is the identification number
of the request. |It has to be unique for each request.

|transaction-type |M |M |Enumeration |30 2+|The type for the transaction -
_authorization_ 
...
|account-holder |M |M |Complex |  2+|The end consumer. 
...
|account-holder.first-name |M |M |Alphanumeric |32 2+|First name of the
end consumer. 

|account-holder.last-name |M |M |Alphanumeric |32 2+|Last name of the
end consumer. 

|account-holder.email |M |M |Alphanumeric |64 2+|Email address of the
end consumer. 
...
|account-holder.address |M |M |Complex |  2+|The address of the
end consumer. 

|account-holder.address.street1 |M |M |Alphanumeric |128 2+|Street name of
the end consumer. 
...
|account-holder.address.city |M |M |Alphanumeric |32 2+|City of the
end consumer. 
...
|account-holder.address.postal-code |M |M |Alphanumeric |16 2+|Postal code
of the the end consumer. 

|shipping |M |M |Complex |  2+|The shipping information of the recipient.
...
|shipping.email |M |M |Alphanumeric |64 2+|Email address of the recipient.
...
|shipping.address |M |M |Complex |  2+|The address of the recipient. 

|shipping.address.street1 |M |M |Alphanumeric |128 2+|Street name of the
recipient. 
...
|shipping.address.city |M |M |Alphanumeric |32 2+|City of the recipient.
...
|shipping.address.postal-code |M |M |Alphanumeric |16 2+|Postal code of
the the recipient. 

|ip-address |M |M |Alphanumeric |15 2+|Global (internet) IP address of the
consumer's computer. |

|order-number |O |O |Alphanumeric |64 2+|Your order number. 
...
|order-items |M |M |Complex |  2+|List of order items. 

|order-items.order-item |M |M |Complex |  2+|Order item. 

|order-items.order-item.quantity |M |M |Numeric |  2+|Quantity of the
order item. 

|order-items.order-item.article-number |M |M |Alphanumeric |256 2+|Article
ID unique for the order. 

|order-items.order-item.name |M |M
|Alphanumeric |256 |Order item name. |

|order-items.order-item.amount |M |M |Numeric |18.3 2+|Price of one order
item unit. 
...
|order-items.order-item.tax-rate |M |M |Numeric |5,2 2+|Tax rate as
...
|additional-merchant-data |O |O |Alphanumeric |  2+|Special data might be
needed for specific merchant categories like travel or event tickets in
addition to order items. The benefit for you is that Klarna
will be better able to score your consumers which in turn will
...
|transaction-id |  |M |Alphanumeric |36 2+|Transaction ID is the unique
identifier for a transaction. It is generated by {payment-provider-name}. 

|request-id |M |M |Alphanumeric |150 2+|Identification number of the
request generated you. It has to be unique for each request. 

|transaction-state |  |M |Alphanumeric |12 2+|Transaction status _success_
/ _failed._ 

|completion-time-stamp |  |M |DateTime |  2+|Timestamp of completion of
request. 

|statuses |  |M |Complex |  2+|List of transaction statuses. 

|statuses.status |  |M |Complex |  2+|Transaction status. 

|statuses.status.code |  |M |Alphanumeric |12 2+|Status code of the
transaction. 

|statuses.status.description |  |M |Alphanumeric |256 2+|Description to
the status code of the transaction. 

|statuses.status.severity |  |M |Enumeration |20 2+|This field gives
information if a status is a _warning_, an _error_ or an _information_.


|statuses.status@provider-code |  |O |Alphanumeric |34 2+|Original Klarna
error code 

|statuses.status@provider-message |  |O |Alphanumeric |  2+|Error message
to be shown to the end consumer as required by Klarna 

|statuses.status@provider-transaction-id |  |O |Alphanumeric |36 2+|Klarna
reservation number 

|notifications.notification@url |O |O |Alphanumeric |256 2+|URL to be used
for the Instant Payment Notification for pending reservations.
It overwrites the notification URL that is set up in your
configuration. 
...
|payment-methods.payment-method-name |M |M |Alphanumeric |15 2+|Name of
...
|Alphanumeric
|36
2+|Unique identifier for a merchant account.
...
|Enumeration
|30
2+|The type for a transaction - _adjust-authorization_.
...
|Complex
|
2+|The end consumer whose data has changed.
...
|Alphanumeric
|32
2+|First name of the end consumer.
...
|Alphanumeric
|32
2+|Last name of the end consumer.
...
|Alphanumeric
|64
2+|Email address of the end consumer.
...
|Complex
|
2+|The address of the end consumer.
...
|Alphanumeric
|128
2+|Street name of the end consumer.
...
|Alphanumeric
|16
2+|Postal code of the the end consumer.
...
|complex
|
2+|The changed shipping data of the recipient.
...
|Alphanumeric
|128
2+|Street name of the recipient.
...
|Alphanumeric
|16
2+|Postal code of the the recipient.
...
|Alphanumeric
|256
2+|Article ID unique for the order.
...
|Numeric
|18,6
2+|Price of one order item unit.
...
|Numeric
|5,2
2+|Tax rate as percentage value or the order item.
...
|Alphanumeric
|36
2+|Transaction ID is the unique identifier for a transaction. It is
generated by {payment-provider-name}.
...
|Enumeration
|12
2+|Transaction status _success_ / _failed_
...
|DateTime
|
2+|Timestamp of completion of request.
...
|Complex
|
2+|List of transaction statuses
...
|Alphanumeric
|12
2+|Status code of the transaction.
...
|Alphanumeric
|256
2+|Description to the status code of the transaction.
...
|Enumeration
|20
2+|This field gives information if a status is a warning, an error or
an information.
...
|Alphanumeric
|
2+|Error message to be shown to the end consumer as required by Klarna
...
|Alphanumeric
|36
2+|Transaction ID of the parent _authorization_ or _adjust-authorization_ transaction.
...
|Enumeration
|15
2+|Name of the payment method: _klarna-invoice_, _klarna-install_
...
|Enumeration
|30
2+|The type for a transaction - _authorization-renewal_
...
|dateTime
|
2+|The new expiration timestamp
...
|Alphanumeric
|36
2+|Transaction ID of the parent _authorization_ or _adjust-authorization_ transaction to be prolonged.
...
|Enumeration
|30
2+|The type for a transaction - _void-authorization_
...
|Alphanumeric
|36
2+|Transaction ID of the parent _authorization_ or _adjust-authorization_ transaction to be voided.
...
|Enumeration
|30
2+|The type for a transaction - _capture-authorization_
...
* For ``klarna-invoice``, this field is **mandatory** in a _capture-authorization_ request and must be set to ``email``.
...
|Alphanumeric
|32
2+|The OCR number identifying the captured order
...
|Alphanumeric
|64
2+|Your order number if not provided with the authorization.
...
|Complex
|
2+|The list of the order items for partial capture.
...
|Must be equal to the price in the _authorization_/_adjust-authorization_ call.
...
|Alphanumeric
|64
2+|Tracking number for the outgoing parcel
...
|Alphanumeric
|2000
2+|URL to the shipping company's page for the outgoing parcel
...
|Alphanumeric
|64
2+|The name of the shipping company for the outgoing parcel
...
- store_pick_up - An online purchase is picked up in your own physical store.
...
address via your own delivery service.
...
|Alphanumeric
|64
2+|Tracking number for the return parcel
...
|Alphanumeric
|2000
2+|URL to the shipping company's page for the return parcel
...
|Alphanumeric
|36
2+|The name of the shipping company for the return parcel
...
|It is generated by {payment-provider-name}.
...
|Alphanumeric
|36
2+|Klarna invoice number (OCR number)
...
|Alphanumeric
|36
2+|Transaction ID of the parent _authorization_ or _adjust-authorization_
transaction.
...
|Alphanumeric
|30
2+|The type for a transaction - _refund-capture_
...
|Complex
|
2+|List of order items to be refunded.
...
|Enumeration
|15
2+|Name of the payment method: _klarna-invoice_ or _klarna-install_.
...
|Alphanumeric
|15
2+|Global (internet) IP address of the consumers computer.
...
|Alphanumeric
|32
2+|First name of the person registered for the current address.
...
|Alphanumeric
|32
2+|Last name of the person registered for the current address.
...
|Alphanumeric
|128
2+|Street name + house number of the found address.
...
|Alphanumeric
|16
2+|Postal code of the found address.
...
|Enumeration
|15
2+|Name of the payment method: _klarna-install_
...
|Alphanumeric
|3
2+|Amount currency of the order.
...
|Alphanumeric
|5
2+|Locale code containing country and language code. E.g. de_de. The
...
|Enumeration
|15
2+|Name of the payment method: klarna-install
...
|Enumeration
|
2+|The state of the response: success, failed.
...
|Alphanumeric
|12
2+|The status code of the response. For the complete list of status codes
please see
...
|Alphanumeric
|256
2+|The description to the status code of the response.
...
|Complex
|
2+|List of available installment plans.
...
|Numeric
|
2+|The Klarna ID uniquely identifying the corresponding payment option /
installment plan.
...
|Alphanumeric
|
2+|Payment option group: invoice for payment on invoice and for "Buy now,
...
|Complex
|
2+|The details of the installment plan.
...
|Alphanumeric
|
2+|Title to display the interest rate in the corresponding language.
...
|Alphanumeric
|
2+|The unit of the interest rate. Usually %.
...
|Alphanumeric
|
2+|Title to display the monthly invoice fee in the corresponding language.
...
|Alphanumeric
|
2+|Title to display the setup fee in the corresponding language.
...
|Alphanumeric
|
2+|Title to display the monthly payment in the corresponding language.
...
|Alphanumeric
|
2+|Title to display the annual percentage rate in the corresponding
language.
...
|Alphanumeric
|
2+|The unit of the annual percentage rate. Usually %.
...
|Alphanumeric
|
2+|Title to display the number of installments (months) in the
...
|Alphanumeric
|
2+|The name of the month in the corresponding language.
...
|Alphanumeric
|
2+|Title to display the total amount in the corresponding language.
...
|Alphanumeric
|
2+|Title to display the payment option in the corresponding language.
...
Extra Merchant Data (EMD) is additional data variables that you
can send in to {payment-provider-name} in the API call in excess of the goods list. The
added value from your perspective is that Klarna will to a
higher extent have the possibility of scoring your consumers,
...
{payment-provider-name} sales account manager in order to clarify what packages are
...
NOTE: You requires the Klarna _Merchant ID_ in order to be able to
use some of the solutions. Please contact your {payment-provider-name} account manager
in order to get the Klarna _Merchant ID_.

[#Klarna_getAddress]
==== get-address for Swedish Consumers

image::images/diagrams/klarna_get-adress.svg[Klarna Workflow get-address for Swedish Customers,height=250]

For the Swedish private persons Klarna requires exact match of the
address data in the _authorization_/_adjust-authorization_ requests. In case
...
{payment-provider-name} and Klarna offer a special online service which retrieves the
...
should be used in the corresponding _authorization_/_adjust-authorization_
request.
...
image::images/diagrams/klarna_installment-calculator.svg[Klarna Workflow of Installment Calculator,height=250]

The Installment Calculator service can be used to retrieve the available
installment plans offered by Klarna. You send the order amount and
...
.XML Klarna Invoice _void-authorization_ Request (Successful)
[source,xml,subs=attributes+]
----
include::{root}/samples/xml/klarna-invoice_void-authorization_request_success_b9e50fc5.xml[]
----

.XML Klarna Invoice _void-authorization_ Response(Successful)
[source,xml,subs=attributes+]
----
...
--
NOTE: Refer to the <<Klarna_Samples_authorization, "XML Klarna Invoice _authorization_ request for DE" sample>> to test these _capture-authorization_ samples.
--

.XML Klarna Invoice Full _capture-authorization_ Request (Successful)
[source,xml,subs=attributes+]
----
include::{root}/samples/xml/klarna-invoice_capture-authorization_request_success_35bdae97.xml[]
----

.XML Klarna Invoice Full _capture-authorization_ Response (Successful)
[source,xml,subs=attributes+]
----
include::{root}/samples/xml/klarna-invoice_capture-authorization_response_success_de_DE_35bdae97.xml[]
----

.XML Klarna Invoice Partial _capture-authorization_ Request (Successful)
[source,xml,subs=attributes+]
----
include::{root}/samples/xml/klarna-invoice_capture-authorization_request_success_b9e50fc5.xml[]
----

.XML Klarna Invoice Partial _capture-authorization_ Response (Successful)
[source,xml,subs=attributes+]
----
