...
. LINE Pay redirects the consumer to the checkout page.
...
. When the transaction becomes payable on the awaiting checkout page, LINE Pay redirects the user to the WPG LinePay plugin URL received from the Merchant when the payment is reserved. 
...
. WPG calls the Confirm Payment API to complete the payment. 
// is the call of the "Confirm Payment API" identical to "send the debit notification"?

==== Recurring Payments

The overall payment process is identical to ``pendig-debit`` and ``debit`` notification, except that LINE Pay's ``payType`` is set to ``preapproved``.
