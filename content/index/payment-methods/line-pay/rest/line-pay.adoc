...
. You redirect the consumer to {payment-gateway-abbr}.
. {payment-gateway-abbr} displays all the available payment methods.
. The consumer selects LINE Pay.
. {payment-gateway-abbr} sends a _pending-debit_ request (_reserve payment_) to LINE Pay.
. LINE Pay includes a redirect URL in the _pending-debit_ response and sends it to {payment-gateway-abbr}. 
. LINE Pay waits for the consumer's interaction.
. {payment-gateway-abbr} asks the consumer to continue with the LINE Pay app.
. The consumer enters the credentials to continue the payment process.
. LINE Pay verfies the consumer's password.
...
. {payment-gateway-abbr} uses {payment-page-v2-abbr} to update the payment process.
. {payment-gateway-abbr} confirms the payment process to LINE Pay.
. LINE Pay processes the confirmation information.
. LINE Pay sends a confirmation response to {payment-gateway-abbr}.
. {payment-gateway-abbr} sends a _debit_ notification to you.
. {payment-gateway-abbr} redirects the consumer to your checkout page.
. You process the _debit_ notification.
