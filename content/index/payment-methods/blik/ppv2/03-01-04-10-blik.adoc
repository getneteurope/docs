...
[#PPv2_blik_TransactionType_refund]
=== Transaction Type _refund_

Basically, _blik_ with {payment-page-v2-abbr} supports _debit_ only. You can integrate a
<<blik_refundProcessUsingtheReSTAPI, refund process for _blik_ via our REST API>>.
...
[#PPv2_blik_JSON_NVPFields]
=== JSON/NVP Field Reference

Here you can find the NVP equivalents for JSON fields (for migrating merchants).

.JSON Structure for blik Requests

[source,json,subs=attributes+]
----
include::{root}/samples/json/WPP_blik_JSON_NVPFields_JSON_StructureFor_blik_Requests.json[]
----

[#PPv2_blik_JSON_NVPFields_Request]
[cols="e,e,e"]
|===
...
[source,json,subs=attributes+]
----
include::{root}/samples/json/WPP_blik_JSON_NVPFieldsRequest_ResponseOnlyFields.json[]
----

[#PPv2_blik_JSON_NVPFields_Response]
[cols="e,e,e"]
|===
| Field (NVP) | Field (JSON) | JSON Parent
