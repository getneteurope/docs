...
image::images/diagrams/blik_debit_rest.png[Payment Process Using Payment Page]
...
. Blik send the payment confirmation to the Payment Gateway (EE)
...
. The payment Page redirect the consumer back
. completion of payment process id displayed to the merchant.
...
image::images/diagrams/blik_refund_rest.png[refund Process Using the ReST API]
