...
debit is created once direct response is received|see debit sample below
...
|reversal-debit  |Yes |Yes |Yes|Reversal is internally called by Payment Gateway (cron job) to revoke pending payments that could not be processed due to timeouts or tech issues. The service runs in asynchronous mode|TBD

|query / GET  |Yes |Yes |Yes||TBD
