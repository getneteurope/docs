...
|get URL | No | No | Yes | A step in a group of transactions where the URL of blik landing page is retrieved for the Consumer to be redirected to.
...
When the notification is sent from Blik to Payment Gateway, then refund-debit transaction is created.
"Partial and multiple refunds are allowed, until the original amount is not exceeded."
|see refund samples below


|void-debit  |Yes |Yes |Yes||TBD

|reversal-debit  |Yes |Yes |Yes|Reversal is internally called by Payment Gateway (cron job) to revoke pending payments that could not be processed due to timeouts or tech issues. The service runs in asynchronous mode|see Void-debit samples below
