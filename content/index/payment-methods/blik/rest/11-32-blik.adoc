...
|refund-debit|Yes |Yes |Yes | When the notification is sent from Blik to EE, then refund-debit transaction is created|see refund samples below


|void-debit  |Yes |Yes |Yes|voids are synchronous. Called by merchant to cancel a transaction.|TBD
