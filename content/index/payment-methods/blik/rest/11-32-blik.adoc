...
image::images/diagrams/blik_debit_rest.svg[Payment Process Using ReST API]
...
image::images/diagrams/blik_debit_pp.svg[Payment Process Using Payment Page]
...
a _Transaction-ID_ generated by blik (_blik-Transaction-ID_) and a value
provided in the field ``descriptor`` of the <<blik_Samples_debit, debit request>>.

The structure of the _blik-Transaction-ID_ is
``blik-YXX-XXX-XXX``. _The _blik-Transaction-ID__ is also displayed in
...
The following tables describe only blik specifics. The following elements
...
along with the _blik-Transaction-ID_. See section <<blik_BankingStatement, Banking Statement>> for details.
...
NOTE: The blik landing page contains the section "Description". This section
