...
_Payment in Installments_ is a type of <<PaymentMethods_PaymentMode_OfflineBankTransfer, Offline Bank Transfer>>. 
...
This table illustrates how _Payment in Installments_ notifications are encoded and which formats and methods can be
...
2+h| Merchant Account ID (MAID) | To be determined
2+h| Username                   | To be determined
2+h| Password                   | To be determined
|===
...
. Consumer selects the payment method _Payment in Installments_, the duration and rate of payment.
...
. Invoice generator informs Getnet.
. Getnet collects the transaction amount from consumer's bank account.
...
The following elements are mandatory *M* or optional *O* in sending a request for _Payment in Installments_ (complete field list available in <<RestApi_Fields, REST API field list>>):
...
|String
|30
|This is the type for a transaction: authorization, capture, void-authorization, refund-capture and credit are supported for _Payment in Installments_.
...
NOTE: We show this table without the _Size_ values as you receive the response directly from _Payment in Installments_ and we cannot provide this information.
...
2+|These fields send the configuration available with _Payment in Installments_. They contain the installment options available to your consumers.
...
_Payment in Installments_ supports two types of installment plan:
...
|amount/@currency|M|String|3|The currency of the purchase. _Payment in Installments_ requires EUR.
...
|rate/@currency|M|String|3|The currency of the rate. _Payment in Installments_ requires EUR.
...
NOTE: We show this table without _Size_ values as you receive the response directly from _Payment in Installments_ and we cannot provide this information.
...
|amount/@currency|String|The currency of the purchase. _Payment in Installments_ requires EUR.
|rate|Decimal|Monthly rate which the consumer wants to pay. It must be in the format of _Money_ (Example: ``25.00``).
|rate/@currency|String|The currency of  the rate. _Payment in Installments_ requires EUR.
...
==== _Payment in Installments_ payment
