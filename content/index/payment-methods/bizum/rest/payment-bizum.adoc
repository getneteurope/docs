...
* *Currencies*: EUR (Getnet is the acquirer and uses EUR, but the settlement bank currency may vary)
* *Countries*: Bizum supports the countries shown in the following table:
...
NOTE: In step 6, you must remove 'amp;' from the redirection URL provided in the ``authorization`` response before using the URL to redirect the consumer to Bizum.
...
2. The Payment Gateway sends the ``capture-authorization`` request to Bizum.
3. The Payment Gateway receives a ``capture-authorization`` response containing the capture status from Bizum.
4. Merchant receives the ``capture-authorization`` response containing the capture status from the Payment Gateway.
5. Merchant displays the capture status to the consumer.
6. Merchant receives a capture notification from the Payment Gateway.
7. Merchant processes the transaction based on the notification.
...
3. The Payment Gateway sends the ``refund-capture`` request to Bizum.
4. The Payment Gateway receives a ``refund-capture`` response containing the refund status from Bizum.
5. Merchant receives a ``refund-capture`` response containing the refund status from the Payment Gateway.
6. Merchant displays the refund status to the consumer.
7. Merchant receives an updated refund status from the Payment Gateway.
8. Merchant processes the updated refund status based on the notification.
...
2. The Payment Gateway sends a getStatus request to Bizum.
3. The Payment Gateway receives a getStatus response containing the payment status.
4. Merchant receives the payment status from the Payment Gateway.
5. Merchant updates the payment status in their system.
6. Merchant receives an updated status notification from the Payment Gateway.
7. Merchant processes the payment status in their system based on the notification.
...
|Alphanumeric
|64
|This is the text representing an order on the consumer's bank statement. Mandatory for ``authorization`` transactions.
...
|Alphanumeric
|64
|The order number from the merchant. It is recommended that the order number be a maximum of 12 characters, otherwise it will be truncated. No special characters are allowed. Mandatory for ``authorization`` transactions but is optional for ``capture-authorization`` and ``refund-capture`` transactions. If used in ``capture-authorization`` or ``refund-capture`` transactions, it must match the order number used in the ``authorization`` request.

|_ip-address_
|C
|Alphanumeric
|45
|The global (internet) IP address of the consumer’s device. For Bizum, this is the IP of the consumer's browser through which the redirection has or will be made. Mandatory for ``authorization`` transactions.
...
|Alphanumeric
|45
|IP address of the consumer’s browser.
Accepted format is IPv4.

Mandatory for ``authorization`` requests and where
