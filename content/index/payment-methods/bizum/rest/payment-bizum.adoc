...
7. Merchant processes the payment status based on the notification.
...
|Alphanumeric
|64
|The order number from the merchant. It is recommended that the order number be a maximum of 12 characters, otherwise it will be truncated. No special characters are allowed. Mandatory for ``authorization`` transactions but optional for ``capture-authorization`` and ``refund-capture`` transactions. If used in ``capture-authorization`` or ``refund-capture`` transactions, it must match the order number used in the ``authorization`` request.
